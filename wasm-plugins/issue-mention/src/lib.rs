...
//!
//! 放行条件（按事件类型区分）：
//! - 新 issue：`action == "opened"` 且含 `issue` 对象 → 检查 `issue.body` 含 `@claude0805`
...
mod tests {
    ...
    fn allows_new_issue_with_mention() {
        let out = evaluate(&event(new_issue_body("请处理 @claude0805 谢谢"))).unwrap();
    ...
    fn allows_comment_with_mention_in_comment_body() {
        let out = evaluate(&event(comment_body("普通正文", "@claude0805 看一下"))).unwrap();
        assert_eq!(out["allow"], true);
    ...
    fn comment_event_ignores_issue_body_mention() {
        ...
        assert!(evaluate(&event(comment_body("正文有 @claude0805", "无关回复"))).is_none());
    }
    ...
    fn opened_event_ignores_comment_field() {
        ...
        b["comment"] = json!({ "body": "@claude0805" });
    ...
    fn created_without_comment_object_is_filtered() {
        ...
        let mut b = new_issue_body("@claude0805");
    ...
    fn filters_wrong_action() {
        ...
        assert!(evaluate(&event(b)).is_none());
        let mut b = comment_body("x", "@claude0805");
